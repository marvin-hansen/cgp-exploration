...
use trait_data_integration::{
    ...
    OhlcvDataStreamProvider,
};

impl<Context> OhlcvDataStreamProvider<Context> for UseImsBinanceDataIntegration
where
    Context: HasBinanceIntegrationFields
        + HasSymbolType<Symbol = String>
        + HasErrorType<Error = ImsDataIntegrationError>
        + HasTimeResolutionType<TimeResolution = TimeResolution>
        + CanProcessEvent
        + CanValidateSymbols
        + HasApiUrl
        + Async
        ...
        + Clone,
{
    ...
    async fn start_ohlcv_data(
        ...
        time_resolution: &Context::TimeResolution,
    ...
    ///
    async fn stop_all_ohlcv_data(context: &Context) -> Result<(), Context::Error> {
        let mut handlers = context.ohlcv_handlers().write().await;
        for (_, handle) in handlers.drain() {
            handle.abort();
            // Clear active OHLCV symbols list when stopping all streams
            context.symbols_active_ohlcv().write().await.clear();
        }
        Ok(())
    }
}
