        ...
        + Clone,
{
    ...
    async fn stop_trade_data(
        ...
        if !not_found_symbols.is_empty() {
            return Err(ImsDataIntegrationError::SymbolNotFound(format!(
                "The following symbols were not found and are NOT active trade data streams: {:?}",
                not_found_symbols
            )));
        }
    ...
    ///
    async fn stop_all_trade_data(context: &Context) -> Result<(), Context::Error> {
        let mut handlers = context.trade_handlers().write().await;
        for (_, handle) in handlers.drain() {
            handle.abort();
            // Clear active trade symbols list when stopping all streams
            context.symbols_active_trade().write().await.clear();
        }
